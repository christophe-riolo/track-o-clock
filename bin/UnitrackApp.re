...
module Logger = (val Timber.Log.withNamespace("Unitrack"));

let fontLocation = "Roboto-Regular.ttf";

module Text = {
  let fontFamily = Font.Family.fromFile(fontLocation);
  let fontSize = 24.;
  let make = (~fontFamily=fontFamily, ~fontSize=fontSize) =>
    Text.make(~fontFamily, ~fontSize);
};
...
let init = app => {
  ...
  Logger.debug(Revery.Environment.executingDirectory);
  Array.iter(
    file => Logger.debug("\t" ++ file),
    Sys.readdir(Environment.executingDirectory),
  );
  if (Sys.file_exists(fontLocation)) {
    Logger.debug("Roboto-Regular.ttf exists");
  };
