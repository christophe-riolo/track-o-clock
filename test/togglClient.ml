...
let post (_t: t) ?(headers: (string*string) list option) ?(body: Body.t option) path =
  ignore (headers, body);
  match path with
  | "/api/v8/time_entries" -> Lwt_result.return (Response.of_string `OK ~body:time_entry)
  | "/api/v8/time_entries/start" -> Lwt_result.return (Response.of_string `OK ~body:time_entry)
